...
public class ExpandListPesananAdapter extends BaseExpandableListAdapter {
    int jumlahProduk = 0;
    double totalHarga = 0;
    private Context context;
    private List<DataPesanan> listHeaderFilter;
    private HashMap<DataPesanan, List<Item>> listChild;
    ...
    public ExpandListPesananAdapter(Context context, List<DataPesanan> listHeader, HashMap<DataPesanan, List<Item>> listChild) {
    ...
    public View getGroupView(final int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        final DataPesanan model = (DataPesanan) getGroup(groupPosition);
        final DataPesanan headerModel = listHeaderFilter.get(groupPosition);
        final List<Item> childModel = listChild.get(listHeaderFilter.get(groupPosition));
        ...
        nama_kk.setText(model.getNamaToko());
        no_pelanggan.setText(model.getJumlahPesanan());
...
//        Log.d("sholeh", listHeaderFilter.get(groupPosition).getId_toko());
        ...
        final CheckBox cbchild;
            final Item childModel = listChild.get(listHeaderFilter.get(groupPosition)).get(childPosition);
...
//        hargaProduk = childModel.getHargaJual();
...
//        headerModel.setIsChecked(isChecked);
        for (int i = 0; i < listChild.get(listHeaderFilter.get(groupPosition)).size(); i++) {
            Item childModel = listChild.get(listHeaderFilter.get(groupPosition)).get(i);
//            childModel.setIsChecked(isChecked);
    ...
    public void setupAllChecked(boolean isChecked) {
        for (int i = 0; i < listHeaderFilter.size(); i++) {
            DataPesanan headerModel = listHeaderFilter.get(i);
//            headerModel.setIsChecked(isChecked);
            List<Item> childMapList = listChild.get(listHeaderFilter.get(i));
            for (int j = 0; j < childMapList.size(); j++) {
                Item childModel = childMapList.get(j);
//                childModel.setIsChecked(isChecked);
            }
        }
        notifyDataSetChanged();
    ...
    public boolean dealOneParentAllChildIsChecked(int groupPosition) {
        for (int j = 0; j < listChild.get(listHeaderFilter.get(groupPosition)).size(); j++) {
            Item childModel = listChild.get(listHeaderFilter.get(groupPosition)).get(j);
